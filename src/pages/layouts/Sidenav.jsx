...
const Sidenav = () => {
  ...
  const localActiveMenuItem = JSON.parse(localStorage.getItem("ACTIVE_SIDE_MENU_ITEM"));
  let storeMenuActiveItem = useSelector(state => state?.activemenu?.activeSideMenuItem);
  ...
  const [activeMenuItem, setActiveMenuItem] = useState({
    ...
    isMenu1Active: localActiveMenuItem ? localActiveMenuItem.isMenu1Active : true,
    subMenu2: localActiveMenuItem ? localActiveMenuItem.subMenu2 : "",
    isSubMenu2Active: localActiveMenuItem ? localActiveMenuItem.isSubMenu2Active : false,
    subMenu3: localActiveMenuItem ? localActiveMenuItem.subMenu3 : "",
    isSubMenu3Active: localActiveMenuItem ? localActiveMenuItem.isSubMenu3Active : false,
  });
  ...
  const handleMenuItemClick = (menu1,isMenu1Active,subMenu2,isSubMenu2Active,subMenu3,isSubMenu3Active) => {
    setActiveMenuItem(activeMenuItem =>  ({ ...activeMenuItem,
                                          menu1: menu1,
        ...
        >
          <div className="sb-sidenav-menu">
            <div className="nav">
              ...
              <Link
                className={`nav-link ${activeMenuItem.menu1 === 'dashboard' ? 'active-me' : ''}`}
                to={{ pathname: "/dashboard" }}
                onClick={() => handleMenuItemClick('dashboard',true,'',false,'',false)}
              >
              ...
              {/* Demand */}
              <Link
                className={`nav-link collapsed ${activeMenuItem.menu1 === 'demand' ? 'active-me' : ''}`}
                to={{}}
              ...
              >
                <nav className="sb-sidenav-menu-nested nav">
                  <Link
                    className={`nav-link ${activeMenuItem.subMenu2 === 'notification' ? 'active-me1' : ''}`}
                    to={{ pathname: "/demandnotification" }}
                    onClick={() => handleMenuItemClick('demand',true,'notification',true,'',false)}
                  >
                    Notification
                  </Link>
                  <Link
                    className={`nav-link ${activeMenuItem.subMenu2 === 'annualdemand' ? 'active-me1' : ''}`}
                    to={{ pathname: "/annualdemand" }}
                    onClick={() => handleMenuItemClick('demand',true,'annualdemand',true,'',false)}
                  >
                    Annual Demand
                  </Link>
                </nav>
              ...
              {/* Stock */}
              <Link
                className={`nav-link collapsed ${activeMenuItem.menu1 === 'stock' ? 'active-me' : ''}`}
                to={{}}
                data-bs-toggle="collapse"
              ...
              >
                <nav className="sb-sidenav-menu-nested nav">
                  <Link
                    className={`nav-link ${activeMenuItem.subMenu2 === 'stocklist' ? 'active-me1' : ''}`}
                    to={{ pathname: "/stocklisting" }}
                    onClick={() => handleMenuItemClick('stock',true,'stocklist',true,'',false)}
                  >
                    Stock List
                  </Link>
                  <Link
                    className={`nav-link ${activeMenuItem.subMenu2 === 'stockentry' ? 'active-me1' : ''}`}
                    to={{ pathname: "/openstockentry" }}
                    onClick={() => handleMenuItemClick('stock',true,'stockentry',true,'',false)}
                  >
                    Stock Entry
                  </Link>
                  <Link
                    className={`nav-link ${activeMenuItem.subMenu2 === 'condemnation-register' ? 'active-me1' : ''}`}
                    to={{ pathname: "/openCondeminationRegister" }}
                    onClick={() => handleMenuItemClick('stock',true,'condemnation-register',true,'',false)}
                  >
